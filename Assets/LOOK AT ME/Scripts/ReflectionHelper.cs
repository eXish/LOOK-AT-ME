...
{
    private static readonly Dictionary<string, Type> _findTypeCache = new Dictionary<string, Type>();
    public static Type FindType(string fullName)
    {
        Type type;
        return _findTypeCache.TryGetValue(fullName, out type) ? type : _findTypeCache[fullName] = AppDomain.CurrentDomain.GetAssemblies().SelectMany(GetSafeTypes).FirstOrDefault(t => t.FullName != null && t.FullName.Equals(fullName));
    }
